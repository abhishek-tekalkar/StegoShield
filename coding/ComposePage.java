	...
	public void actionPerformed(ActionEvent ae) {
		if (ae.getSource() == picture_load_button) {
			...
			if (starflag == 0) {
				...
				try {
					if (filepath.isEmpty() && (secret_text.getText()).isEmpty())
						JOptionPane.showMessageDialog(null, "image and secret info are empty. enter them");
					else if (secret_info.isEmpty() && !filepath.isEmpty())
						JOptionPane.showMessageDialog(null, "enter secret info");
					else if (filepath.isEmpty() && !(secret_text.getText()).isEmpty())
						...
						BufferedImage bufferedThumbnail = new BufferedImage(thumbWidth, thumbHeight, BufferedImage.TYPE_INT_RGB);
						bufferedThumbnail.createGraphics().drawImage(thumbImage.getScaledInstance(thumbWidth, thumbHeight, Image.SCALE_SMOOTH), 0, 0, null);
						ImageIO.write(bufferedThumbnail, "jpg", f);
